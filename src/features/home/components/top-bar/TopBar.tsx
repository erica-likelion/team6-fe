...
import { Icon } from '@components/Icon'; // (default export면: import Icon from '@components/Icon')
  ...
  return (
    <div className={`flex h-14 items-center justify-between px-5 ${className}`}>
      <div className="flex items-center gap-2">
        <span className="text-[20px] leading-none font-extrabold">{title}</span>
        <span className="inline-flex items-center gap-1 text-[13px] text-gray-700">
          <Icon icon="map-marker_fill" className="h-[14px] w-[14px]" />
          {location}
        </span>
      </div>
